...
exports.update = async function (req, res) {
  try {
    ...
    if (userId.toString() !== id.toString()) return res.status(401).
      json(
        { message: 'Sorry, you don\'t have the permission to upd this data.' })

    const user = await User.findByIdAndUpdate(id, { $set: update },
      { new: true })

    //if there is no image, return success message
    if (!req.file) return res.status(200).
      json({ user, message: 'User has been updated' })

    //Attempt to upload to cloudinary
    const result = await uploader(req)
    const user_ = await User.findByIdAndUpdate(id, { $set: update },
      { $set: { profileImage: result.url } }, { new: true })
...
exports.destroy = async function (req, res) {
  try {
    ...
    //Make sure the passed id is that of the logged in user
    if (user_id.toString() !== id.toString()) return res.status(401).
      json(
